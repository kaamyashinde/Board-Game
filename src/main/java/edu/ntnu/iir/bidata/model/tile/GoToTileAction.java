...
/**
 * A tile action that moves a player to a specific tile on the board.
 * This action can be used to move players forward or backward by specifying
 * the target tile's ID. If the target tile doesn't exist, a GameException is thrown.
 *
 * @author kaamyashinde
 * @version 1.0
 */
public class GoToTileAction implements TileAction {
    ...
    /**
     * Executes the action by moving the player to the target tile.
     * The player will move forward through the tiles until reaching the target.
     *
