...
/**
 * A tile action that allows a player to switch positions with the player in front of them.
 * This action adds a competitive element to the game by allowing players to catch up
 * to others who are ahead. The action will only take effect if there is at least one
 * player ahead of the current player.
 *
 * @author kaamyashinde
 * @version 1.0
 */
public class SwitchPositionAction implements TileAction {
    ...
    /**
     * Constructs a new SwitchPositionAction with the list of all players in the game.
     * This list is needed to determine which player is in front of the current player.
     *
     ...
     * This description is used to inform players about what will happen
     * when they land on a tile with this action.
     *
    ...
    /**
     * Executes the position switch action. If there is a player ahead of the current player,
     * their positions will be swapped. If there is no player ahead, no action is taken.
     *
    ...
    /**
     * Helper method to find the player who is closest in front of the current player.
     * This method searches through all players and finds the one who is ahead of the
     * current player by the smallest number of positions.
     *
